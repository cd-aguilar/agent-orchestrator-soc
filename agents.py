...
def research_node(state: TriageState) -> dict:
    ...
    prompt = (
        ...
        "recommends.\n\n"
        "Important: the Context describes what a technique typically looks "
        "like — it is reference material, not a description of this alert. "
        "Only state that a specific detail (e.g. a regular/periodic "
        "interval, a data volume spike, a specific protocol) is present if "
        "the Alert text actually says so. If the alert only partially "
        "matches a technique (e.g. it shows an outbound connection but "
        "doesn't confirm beaconing regularity or a data volume), say that "
        "explicitly instead of asserting the full technique description as "
        "observed fact.\n\n"
        f"Context:\n{context}\n\n"
        f"Alert:\n{state['alert_raw']}"
    )
